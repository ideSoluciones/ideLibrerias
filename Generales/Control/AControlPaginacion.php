...
class AControlPaginacion{
	...
	static $noPaginacion=0;
	
	function __construct($id="",$propiedades=array()){
		$this->sesion=Sesion::getInstancia();
		if(is_string($id)){
			if(strlen($id)>0){
				$this->id=$id;
			}else{
				$this->id="paginacion".$this->noPaginacion;
				$this->noPaginacion++;
			}
		}else{
			$this->id="paginacion".$this->noPaginacion;
		...
		if(isset($propiedades["base"])){
			...
			$this->basePotenciaSaltosPaginas=intval($this->sesion->leerParametroInterno("{$this->id}{$this->casoUso}","basePotenciaSaltosPaginas"));
			if($this->basePotenciaSaltosPaginas<=1){
				$this->basePotenciaSaltosPaginas=2;
			}
		}
		$this->paginaActual=intval($this->sesion->leerParametroInterno("{$this->id}{$this->casoUso}","paginaActual"));
		...
		if(isset($propiedades["registrosPorPagina"])){
			...
			$this->registrosPorPagina=intval($this->sesion->leerParametroInterno("{$this->id}{$this->casoUso}","registrosPorPagina"));
	...
	function setTotalRegistros($totalRegistros){
		if(intval($totalRegistros)<0){
			$this->totalRegistros=intval($this->sesion->leerParametroInterno("{$this->id}{$this->casoUso}","totalRegistros"));
		}else{
			$this->totalRegistros=$totalRegistros;
			$this->sesion->escribirParametroInterno("{$this->id}{$this->casoUso}","totalRegistros",$this->totalRegistros);
		}
		$this->totalPaginas=ceil(intval($this->totalRegistros)/$this->registrosPorPagina);
