var headerFlotante = null;
var floating_header = function() {

headerFlotante = document.createElement('table');
headerFlotante_height = 0;
...
this.setheader = function() {
        ...
        var theady_max = this.getXY(this.table_obj.getElementsByTagName('THEAD')[0]).y + this.table_obj.offsetHeight - headerFlotante_height;
        if ( screenpos > this.theady && screenpos < theady_max ) {
            headerFlotante.style.top=Math.round(screenpos);
            headerFlotante.style.display = 'block';
            headerFlotante_height = header.offsetHeight;
            headerFlotante.style.left= header.offsetLeft;
        }
        else {
            headerFlotante.style.display = 'none';
        }
}
...
this.build_header = function() {
    ...
    thead.id = 'copyrow';
    headerFlotante.style.position='absolute';
    headerFlotante.style.display='none';
    headerFlotante.appendChild(thead);
    headerFlotante.style.width = this.table_obj.offsetWidth;
    headerFlotante.style.left = findPosX(this.table_obj);
    ...
    for ( var x = 0; x < copyths.length; x++ ) {
        ...
        if ( x == copyths.length - 1 ) {
            headerFlotante.style.paddingBottom = pb;
            headerFlotante.style.borderBottom = bb;
        }
    }
    this.addclass(headerFlotante, 'main');
    document.body.appendChild(headerFlotante);
/*
	b=$('body').append('<table id="micopia" style="display: none;"/>');
	a=$(thead).clone();
	c=$('#micopia').append(a);
	headerFlotante=c;
*/
